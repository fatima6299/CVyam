...
router.get('/paid', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT * FROM orders WHERE client_email = $1 AND status = 'paid' ORDER BY date DESC LIMIT 1",
      [email]
    )
...
router.post('/', async (req, res, next) => {
  try {
    ...
    await pool.query(
      ...
      [id, client.name, client.email, client.mode || 'auto', method, amount || 0, date, JSON.stringify(cvData || {})]
    )
